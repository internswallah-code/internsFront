...
const CourseDetail = () => {
  ...
  const [courses, setCourses] = useState([]); // for sidebar offers
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // ✅ applied logic preserved
  ...
  const applyForCourse = () => {
    ...
    localStorage.setItem(
      "appliedCourses",
      JSON.stringify(updatedAppliedCourses),
    );
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setCourses(listData.filter((c) => c._id !== id));
      } catch (err) {
      ...
      <div className="w-full xl:w-3/4 bg-white shadow-2xl rounded-2xl p-6 sm:p-8 h-full mb-8 xl:mb-0 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-transparent">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-gray-900 text-base md:text-lg border-t pt-6">
          <div className="flex flex-col gap-4">
            ...
            <p className="flex items-center gap-2 md:gap-3 text-gray-700">
              <FaCalendarAlt className="text-purple-500" />
              <span className="font-bold">Listed:</span>{" "}
              {formatPostedDate(course.createdAt)}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <p className="flex items-center gap-2 md:gap-3">
              ...
              <span className="font-bold">Course Type:</span> {course.type}
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 text-gray-800">
          ...
          <p className="text-base md:text-lg">{course.description}</p>
        </div>
